...
const TableTopupAdmin = (props) => {
  ...
  async function handleVerif(item) {
    ...
    let response = await axios
      ...
      .put("http://34.172.96.175/api/customer/topup/detail/aproval/" + item.id)
      .catch((err) => alert(err));
  ...
  useEffect(() => {
    const dataRes = async () => {
      let response = await axios
        // .get("http://localhost:8081/api/customer/topup/detail/all")
        .get("http://34.172.96.175/api/customer/topup/detail/all")
        .catch((err) => alert(err));
      console.log("--------------");
      console.log(response);
      console.log("--------------");
